...
def track():
    ...
    try:
        ...
        rdata = requests.get(f"http://ip-api.com/json/{str(tgt)}")
        data = rdata.json()
        target = data['query']
        ...
        with open(f"./Program/track_res/{filename}.hikariconsole", 'w+') as f:
            f.write(f"IP tracker results for {tgt}\n\nTarget {target}\nISP: {isp}\nOrganization: {org}\n:City: {city}\nCountry: {region}\nPostal Code: {postal_code}\nLongtitude: {long}\nLatitude: {lat}\nTimezone: {timezone}\n" + "*" * 10 + " Generated by Hikari console. " + "*" * 10)
            print(Fore.GREEN + f"Done. Saved to Program/track_res/{filename}.hikariconsole")
            os.system("sleep 5")
            main.main()
